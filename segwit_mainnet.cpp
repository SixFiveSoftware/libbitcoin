...
ec_compressed getPublicKey() {
  std::string mnemonic = "crowd carry sea tribe crater frozen involve kitchen guilt shrimp winter store";
  data_chunk seed = to_chunk(decode_mnemonic(split(mnemonic)));
  wallet::hd_private privateKey = wallet::hd_private(seed);
...
int main() {
  ...
  std::cout << "Payment Address: " << payment_address(P2WPKH).encoded() << std::endl;
}